 ...
 *   rule-only 1.0/0.0 | ML-only 0.0/1.0 | legacy 0.6/0.4 | selected 0.5/0.5
 * See hybrid_weight_ablation.json (npm run ml:hybrid-ablation).
 */
const HYBRID_RULE_WEIGHT = Number(process.env.HYBRID_RULE_WEIGHT || 0.5);
const HYBRID_ML_WEIGHT = Number(process.env.HYBRID_ML_WEIGHT || 0.5);
...
/** Section 13.2 — final runtime defaults selected for deployment: Warning >= 15, Dangerous >= 50. */
...
const RISK_THRESHOLDS = {
  warningMin: Number(process.env.RISK_THRESHOLD_WARNING || 15),
  dangerousMin: Number(process.env.RISK_THRESHOLD_DANGEROUS || 50),
};
