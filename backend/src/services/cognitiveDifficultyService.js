...
function recommendNextDifficulty(context) {
  ...
  if (reasons.length) nextDifficulty = 'easy';
    ...
    const comparableSuccesses = (context.recentCognitiveAttempts || []).filter((attempt) => attempt.difficulty === 'easy' && attempt.completionStatus === 'completed' && Number(attempt.accuracy) >= 0.8).length;
  ...
  return { nextDifficulty, explanation: { currentDifficulty: context.currentDifficulty, accuracy: context.accuracy, completionStatus: context.completionStatus, recentComparableSuccesses: (context.recentCognitiveAttempts || []).filter((a) => a.difficulty === context.currentDifficulty && a.completionStatus === 'completed' && Number(a.accuracy) >= 0.8).length, finalEmotion: context.finalEmotionalState, risk: context.riskLevel, conversationEngagement: context.conversationEngagement, nextDifficulty, reasons, responseTimeUsedForDecision: false } };
}
